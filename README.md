    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        body {
            ...
            height: 100vh;
            width: 100vw;
            background: radial-gradient(circle at center, #3d0a0a 0%, #080202 100%);
        }

        #star-canvas, #falling-roses-canvas { position: fixed; inset: 0; pointer-events: none; z-index: 1; }
        #falling-roses-canvas { z-index: 2; opacity: 0; transition: opacity 2s; }
        ...
        .volume-icon { font-size: 5rem; margin-bottom: 20px; filter: drop-shadow(0 0 20px var(--rose-red)); animation: pulse 2s infinite; }
        ...
        .race-bar-fill { width: 0%; height: 100%; background: var(--gold); box-shadow: 0 0 15px var(--gold); transition: width 0.3s; }

        /* --- HEART SCREEN --- */
        #heart-screen {
            position: fixed; inset: 0; background: #000; display: none;
            flex-direction: column; justify-content: center; align-items: center;
            z-index: 5000; opacity: 0; transition: opacity 1s;
        }
        ...
        /* --- SPACE SCREEN --- */
        #space-screen { display: none; position: relative; width: 100%; height: 100%; opacity: 0; transition: opacity 2s; z-index: 5;}
        ...
        .pop-out { animation: burst 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }
        @keyframes burst {
        ...
        .narrative-box {
            ...
            padding: 20px; background: rgba(0,0,0,0.6); border-radius: 20px;
            backdrop-filter: blur(5px); border: 1px solid rgba(255,255,255,0.1);
        }

        .narrative-text {
            font-family: 'Dancing Script', cursive; font-size: 1.3rem; color: #ffccd5;
            line-height: 1.4;
        }
        ...
        .modal-content {
            background: #120505; padding: 25px; border-radius: 30px;
            border: 1px solid var(--gold); text-align: center;
            max-width: 450px; width: 100%; box-shadow: 0 0 50px rgba(251, 192, 45, 0.15);
        }
        .modal-img {
            width: 100%; max-height: 55vh; object-fit: contain;
            border-radius: 20px; background: #111; border: 1px solid #333;
        }
        ...
        .giant-emoji-rose {
            ...
            filter: drop-shadow(0 0 30px var(--rose-red));
            animation: float-rose 3.5s ease-in-out infinite;
            z-index: 15;
        }
        ...
        .btn-main {
            ...
            color: #000; padding: 15px 40px; font-family: 'Cinzel', serif;
            font-weight: 700; border-radius: 50px; cursor: pointer;
            margin-top: 25px; transition: 0.4s; letter-spacing: 1px;
            z-index: 20;
        }
...
<body>
    ...
    <!-- Use a direct stream link for Google Drive via a proxy service -->
    <audio id="bgMusic" loop preload="auto">
        <source src="https://drive.google.com/uc?export=download&id=136qhu9wSYdba5TWYRcE8H5vwYd2CkzDi" type="audio/mpeg">
    </audio>

    <div id="volume-screen">
        <div class="volume-card">
            <div class="volume-icon">❤️</div>
            <p class="fucchi-msg">I have made something small but beautiful for my fucchi... raise the volume and start it okii? ❤️</p>
            <div class="race-bar-container"><div class="race-bar-fill" id="raceFill"></div></div>
            <button class="btn-main" id="startBtn">START IT OKII</button>
        </div>
    </div>

    <div id="heart-screen">
        <div class="heart-container" id="heartTrigger"><div class="heart-shape"></div></div>
        <h2 style="font-family:'Dancing Script'; font-size: 2.2rem; margin-top: 40px; color: #ffccd5;">Open My Heart...</h2>
    </div>

    <div id="space-screen">
        <div class="narrative-box">
            <p class="narrative-text" id="spaceStory">
                ohhh wait k ho yo..?? Roses in my heart..? Huh..?? Waaa collect them all jenishi all for youuuuu 😂❤️
            </p>
            <p style="font-size: 0.85rem; margin-top: 10px; color: var(--gold); opacity: 0.8;">
                Click each rose to relive a memory
            </p>
        </div>
    ...
    <div class="modal" id="photoModal">
        <div class="modal-content">
            <div id="loadingText" style="color: var(--gold); margin-bottom: 15px; font-size: 0.9rem;">Capturing memory...</div>
            <img id="modalImg" src="" class="modal-img">
            <p id="modalMsg" style="font-family:'Dancing Script'; font-size:1.6rem; margin: 20px 0; color: #ffccd5;"></p>
            <button class="btn-main" style="padding: 10px 25px;" onclick="closeModal()">Back to my heart 🌹</button>
        </div>
    </div>

    <div id="finale-screen">
        <div class="giant-emoji-rose">🌹</div>
        <h1 style="font-family:'Dancing Script'; font-size: 3.5rem; color:var(--soft-pink); text-shadow: 0 0 20px rgba(229, 57, 53, 0.4);">Happy Rose Day</h1>
        <h2 style="font-family:'Cinzel'; color:var(--gold); margin-bottom: 20px; font-size: 1.8rem; letter-spacing: 2px;">Meri Pyari Jainu</h2>
        <p style="padding:0 25px; font-family:'Dancing Script'; max-width:550px; color:#fff; line-height: 1.6; font-size: 1.5rem;">
            "Every single rose you just collected represents a beautiful moment of us. These memories are the most precious things I carry in my heart. Happy Rose Day, my favorite person."
        </p>
        <button class="btn-main" onclick="location.reload()">REPLAY THE MAGIC? ❤️</button>
    </div>
    ...
    <script>
        ...
        class Star {
            ...
            draw() {
                ctx.fillStyle = `rgba(251, 192, 45, ${Math.abs(this.alpha)})`;
        ...
        class FallingRose {
            ...
            draw() {
                ...
                roseCtx.fillText('🌹', 0, 0);
                roseCtx.restore();
            }
        }

        for (let i = 0; i < 120; i++) stars.push(new Star());
        for (let i = 0; i < 40; i++) fallingRoses.push(new FallingRose());
        ...
        const memories = [
            { msg: "You In This Black Sari - Ufff 😭😭😭❤️", id: "1PUkoWK10lRIhL4CpS75SP2miiQTUZG64" },
            { msg: "This Smile ❤️", id: "1RfiryDNKE-6sTupJgEJVLFF4gAcvxRIk" },
            ...
            { msg: "Hamro Peace Treaty 😂😭❤️", id: "1EFp_Xrz95XayoY9dZkGtSAz1nohBwXXw" },
            { msg: "Us At Manakamana", id: "1pFBAhzjbvVUU2bp9RcbA7YF90GsxdlKA" },
            { msg: "The Day Hamare Ghar Wale Chining Uh 🙂❤️", id: "1N-32gSkueiP6bIeEjpROWzJ5kfEUU2wp" },
            { msg: "Bigrya Phone Ma K Herdai Ho Khaiii🙂", id: "1EHKZYct1YEwCbCU958eOHVUfBNTnnkJV" },
            ...
            { msg: "Oyeee Ko Ho Yo..?? 😂❤️", id: "1aXo3TKs4O6CaO-fhkRAgqYRHeVjp2lV4" },
            { msg: "Heart Bananeki Kosish❤️", id: "1AqZehA_mTBpQnI6nfonWwJ-JKyIaCm2J" },
            { msg: "Current Us Ekdam Babal❤️", id: "1aCt8BTCc-K8TBUFesP47y6chWHtY4AWD" }
        ];
        ...
        document.getElementById('startBtn').onclick = function() {
            ...
            audio.play().catch(() => {
                console.log("Audio waiting for more interaction...");
            });

            const interval = setInterval(() => {
                width += 1.5; fill.style.width = width + '%';
                if(width >= 100) { 
        ...
        document.getElementById('heartTrigger').onclick = function() {
            // Stronger attempt to play audio on this interaction
            audio.play();
            explodeHeart();
        };
        ...
        function explodeHeart() {
            ...
            heart.style.transition = '0.6s'; heart.style.transform = 'scale(50)'; heart.style.opacity = '0';
            setTimeout(() => { document.getElementById('heart-screen').style.display = 'none'; initSpace(); }, 500);
        }
        ...
        class Rose {
            ...
            update() {
                ...
                if (this.y <= 130 || this.y >= window.innerHeight - ROSE_SIZE) this.vy *= -1;
        ...
        function closeModal() {
            ...
            rosesCollected++;
            if(rosesCollected === 1) {
                document.getElementById('spaceStory').innerText = "But each rose contains a 